...
class TestParsePdf:
    ...
    @patch("app.ingestion.parser.Path.exists", return_value=True)
    @patch("app.ingestion.parser.open", create=True)
    @patch("app.ingestion.parser.LlamaCloud")
    @patch("time.sleep", return_value=None)
    def test_returns_list_of_documents(self, mock_sleep, MockLlamaCloud, mock_open, mock_exists):
        """parse_pdf should return a list of LlamaIndex Document objects via the new SDK."""
        from app.ingestion.pipeline import parse_pdf

        mock_client = MockLlamaCloud.return_value
        mock_client.files.create.return_value.id = "file_123"
        mock_client.parsing.create.return_value.id = "job_123"
        mock_client.parsing.get_status.return_value.status = "SUCCESS"
        mock_client.parsing.get_result.return_value.markdown = "# Revenue\n| Year | Amount |\n|---|---------|\n| FY24 | 12000 |\n\n## Highlights\nGMV grew by 25%."
        ...
        mock_client.parsing.create.assert_called_once()

    @patch("app.ingestion.parser.Path.exists", return_value=True)
    @patch("app.ingestion.parser.open", create=True)
    @patch("app.ingestion.parser.LlamaCloud")
    @patch("time.sleep", return_value=None)
    def test_llamaparse_uses_markdown_result_type(self, mock_sleep, MockLlamaCloud, mock_open, mock_exists):
        """The parser must request result_type='markdown' from the new SDK."""
        from app.ingestion.pipeline import parse_pdf

        mock_client = MockLlamaCloud.return_value
        mock_client.parsing.get_status.return_value.status = "SUCCESS"
        mock_client.parsing.get_result.return_value.markdown = ""
        ...
        mock_client.parsing.create.assert_called_once()
        call_kwargs = mock_client.parsing.create.call_args.kwargs
        assert call_kwargs.get("result_type") == "markdown"

    @patch("app.ingestion.parser.Path.exists", return_value=True)
    @patch("app.ingestion.parser.LlamaCloud")
    @patch("time.sleep", return_value=None)
    def test_parse_pdf_raises_runtime_error_on_failure(self, mock_sleep, MockLlamaCloud, mock_exists):
        """parse_pdf should wrap SDK errors or job failures in RuntimeError."""
        from app.ingestion.pipeline import parse_pdf

        mock_client = MockLlamaCloud.return_value
        mock_client.parsing.get_status.return_value.status = "FAILURE"
